...
import state, { setState, multiSubscribe, setStateProp, subscribe } from "../../../state";
...
import getTileTypes, { ChipOutputTileType, TileType } from "../../tiles/tile_types";
...
export default class TileSelector extends SelectorMenu {
    ...
    constructor() {
        super(
            ...
            (i, tileSize) => {
                ...
                if (tileOn instanceof ChipOutputTile) {
                    const outputType = state.selectableTiles[i] as ChipOutputTileType;
        ...
        multiSubscribe(["chipEditor", "chipGridMode", "currentChipGrid", "editedChip"], () => {
                            ...
                            const selectableTiles: TileType[] = [
                                ...
                                .map(({ name, tile }) =>
                                    tile instanceof ChipOutputTile
                                        ? new ChipOutputTileType(name, tile.type, tile.hue)
                                        : new TileType(name, tile.type)
                                );

                            value.push(...selectableTiles, new TileType("Block", StructureTile));
                        }
                    }
                } else {
